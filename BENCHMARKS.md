...
The standing recipe is DP8 attention with EP8 experts, DSpark k=5 greedy decoding, a batched-token budget of 8,192, and GPU memory utilization 0.85. Its selection combines bounded topology, cache-placement, token-budget, speculative-width, and TP4 challenger campaigns.
...
Each cell contains three six-turn sessions. Turn 1 is cold. The warm aggregate contains turns 2 through 6, giving 15 observations per arm and context. Normalized wall is calculated for every request as `ttfvt_s + 800 / tps_decode`, then aggregated.

| Context | Arm | Normalized 800 wall p50 | Normalized 800 wall mean | Decode p50 | TTFT p50 | Warm stalls over 3 s |
| --- | --- | ---: | ---: | ---: | ---: | ---: |
...
| Context | Normalized p50 delta | Decode p50 delta | TTFT p50 delta |
| --- | ---: | ---: | ---: |
...
The output lengths differed between arms. Normalized wall supplies the equal-answer-length comparison. The pooled values are descriptive because turns within a session share cache placement. Saturation, aggregate throughput, accuracy, and model quality remain unmeasured.
...
| Path | Sessions per context | 128K-class late median TTFT range | 256K-class late median TTFT range |
| --- | ---: | ---: | ---: |
...
| ---: | ---: | --- |
| 8,192 | 20,971,965 | Preserved long-context headroom and became the standing choice |
...
k=7 improved normalized serial wall versus k=5 by 13.0% at 128K and 6.4% at 256K, but regressed more under concurrent long-context load and consumed more KV headroom. k=5 retained most of the decode gain with the stronger operating balance.
...
A final fixed-output challenger compared DP8 with two TP4 and EP4 replicas. The TP4 profile improved decode time per output token by 15.3% at 1M, but full wall time was 69.58 seconds versus 67.05 seconds for DP8, a 3.8% regression.

The TP4 profile also showed a sticky slow warm range of 16.74 to 24.49 seconds. Under four forced concurrent 1.03M sessions on one replica, only one session completed all turns before the ten-minute stop. The planned mixed 12-session arm was skipped after that safety stop. DP8 remained the standing topology.
...
- Cold topology, token-budget, DSpark-width, and TP4 challenger values are summary-only records from retained campaigns.
- Cold TTFT, warm session latency, normalized equal-length wall, and decode-window rate answer different questions and are not interchangeable.
